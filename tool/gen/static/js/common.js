...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (fullscreenBtn) {
        fullscreenBtn.addEventListener('click', function() {
            const popPic = document.querySelector('.pop-pic');
            const img = popPic.querySelector('img');
            
            // 检查是否已经是全屏状态
            if (!document.fullscreenElement) {
                // 进入全屏模式
                if (popPic.requestFullscreen) {
                    popPic.requestFullscreen();
                } else if (popPic.webkitRequestFullscreen) { /* Safari */
                    popPic.webkitRequestFullscreen();
                } else if (popPic.msRequestFullscreen) { /* IE11 */
                    popPic.msRequestFullscreen();
                }
                
                // 全屏样式
                popPic.style.width = '100%';
                popPic.style.height = '100%';
                popPic.style.position = 'fixed';
                popPic.style.top = '0';
                popPic.style.left = '0';
                popPic.style.backgroundColor = '#000';
                popPic.style.zIndex = '9999';
                popPic.style.display = 'flex';
                popPic.style.flexDirection = 'column';
                popPic.style.justifyContent = 'center';
                popPic.style.alignItems = 'center';
                ...
                // 隐藏提示信息
                const tips = popPic.querySelector('.tips');
                if (tips) {
