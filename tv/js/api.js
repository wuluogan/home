...
async function handleApiRequest(url) {
    ...
    try {
        if (url.pathname === '/api/search') {
            ...
            try {
                // 添加鉴权参数到代理URL
                // 在api.js中找到使用PROXY_URL的地方，修改为使用CorsProxy
                ...
                // 修改为：
                // 在详情API处理部分添加CORS代理支持
                // let response;
                // if (PROXY_URL.startsWith('cors-proxy://')) {
                //     // 使用CORS代理
                //     const targetUrl = apiUrl; // 直接使用原始API URL
                //     response = await window.CorsProxy.fetchWithCorsProxy(targetUrl, {
